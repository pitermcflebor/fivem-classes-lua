...
function Vehicle:GetBrokenWindows()
	local windows = {}
	for i=0, 14, 1 do
...
function Vehicle:SetProperties(props)
	if self:Exists() then
		if props.model == self.model then
			...
			-- damages
			SetVehicleBodyHealth(self.id, props.bodyHealth+0.0 or cProps.bodyHealth)
			SetVehicleDirtLevel(self.id, props.dirtLevel+0.0 or cProps.dirtLevel)
			SetVehicleEngineHealth(self.id, props.engineHealth+0.0 or cProps.engineHealth)
			SetVehiclePetrolTankHealth(self.id, props.petroltank+0.0 or cProps.petroltank)
